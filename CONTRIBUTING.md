# Contributing to @plasius/gpu-debug
...
`@plasius/gpu-debug` provides an opt-in GPU runtime instrumentation package:

- caller-tracked allocation accounting,
- queue, dispatch, and frame-budget summaries,
- optional host-supplied hardware hints,
- local-first debug snapshots for WebGPU runtimes.
...
- `feat: add queue-pressure snapshot fields`
- `fix: ignore aborted dispatch samples`
- `docs: expand hardware-hint examples`
- `refactor: simplify allocation summary generation`
- `test: add cases for bounded sample retention`
- `chore: bump dev deps`
...
- Add tests covering bounded histories, disabled mode, and snapshot semantics.
- Keep inferred metrics clearly labeled; do not imply unavailable hardware
  counters are authoritative.
- Document any data-contract or instrumentation-scope changes in ADRs/TDRs when
